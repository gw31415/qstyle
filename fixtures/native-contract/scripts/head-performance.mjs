...
const fixtureRoot = resolve(dirname(scriptPath), '..');
const outputPath = process.env.HEAD_PERFORMANCE_OUTPUT ?? '/private/tmp/qstyle-head-performance.json';
...
function optionsFor(mode, stream) {
  ...
  if (mode === 'direct') {
    ...
    // Native head placement requires a complete, in-order HTML document.
...
async function runWorker(mode, payloadBytes) {
  ...
  // The candidate server runtime may retain a MessageChannel after rendering;
...
async function runHarness() {
  ...
  const report = {
    benchmark: 'qstyle native head placement observation',
    startedAt,
    candidate: '@qwik.dev/core resolved from fixtures/native-contract/node_modules',
    protocol: {
      experimental: { suspense: true, errorBoundary: false },
      manifest: 'empty source-level manifest',
      delayedPromiseMs: 25,
